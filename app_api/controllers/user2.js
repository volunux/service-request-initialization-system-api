...
module.exports = (opt) => {
	...
	'entryExists' : (req , res , next) => {	entry = req.user._id;
		...
		if (req.user) {
			...
			'entries' : (req , res , next) => { let customQuery = {};
...
/*							.populate({'path' : 'author' , 'select' : 'firstName lastName studentIdentificationNumber level -_id'})
	...
	'entryDeleteSubmit' : (req , res , next) => { let entry = req.params.entry;
		...
		if (req.params && req.params.entry) {
			...
			$Model.findOneAndDelete({'_id' : entry})
																								.lean({})
																							...
																							if (entryResult) {		return response.response(res , 204 , {'message' : `Entry successfully removed from the record.`});	}		});		}
				...
				if (req.user) {
						...
						$Model.findOneAndUpdate({'_id' : entry} , 
																		...
																		{'new' : true })
																										...
																										.select('profileStatus signature role profilePhoto')
			...
			console.log(token);
			 ...
			 $Model.findOne({ 'resetPasswordToken' : token , 'resetPasswordExpires' : { $gt : Date.now() } })
			 		...
			 		.exec((err , entryResult) => {
									...
									if (!entryResult) { return response.response(res , 404 , {'message' : `Password reset token is invalid or has expired. Kindly request for another password reset token`});	}
		...
		'resetPasswordSubmit' : (req , res , next) => { let token = req.params.token;
					...
					$Model.findOne({ 'resetPasswordToken' : token , 'resetPasswordExpires' : { $gt: Date.now() } })
						...
						if (!entryResult) { return response.response(res , 400 , {'message' : `Password reset token is invalid or has expired. Kindly request request for another password reset token.`});	}
									...
									entryResult.setPassword(req.body.password);

									entryResult.resetPasswordToken = undefined;
									
									entryResult.resetPasswordExpires = undefined;

									return entryResult.save((err , entryResult1) => {

													if (err) {	return response.errResponse(res , 400 , err);	}

													else { mailer.successfulReset(req , res , next , entryResult1 , done);

														return response.response(res , 200 , {'message' : `Password successfully changed and updated.`});		} });		});
		} ,

		'entryforgotPasswordSubmit' : (req , res , next) => { let authorization = {'token' : '' };

				$Model.findOne({ 'emailAddress' : req.body.emailAddress })

					.select('resetPasswordToken resetPasswordExpires events.EventEmitter();ailAddress _id')
					...
					.exec((err, entryResult) => {
							...
							if (entryResult) { crypto.randomBytes(20 , (err , buf) => {	authorization.token = buf.toString('hex');	});

																	entryResult.resetPasswordToken = authorization.token;

																	entryResult.resetPasswordExpires = Date.now() + 3600000;

																	return entryResult.save((err , entryResult1) => { mailer.forgotPassword(req , res , next , authorization.token , entryResult1);

																		console.log(entryResult1);
