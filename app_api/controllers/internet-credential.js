...
module.exports = (opt) => {
	...
	const RequestCredential = opt.RequestCredential;
		...
		if (entryResult.status == 'Rejected') {		return response.response(res , 400 , {'message' : `${opt.second} entry cannot be set to rejected more than once.`});	}
			...
			$Model
						.findOneAndUpdate({'slug' : entry} , 
															...
															{'$set' : {'status' : status , 'entryHandler' : req.body.author , 'requestPassword' : req.body.requestPassword , 'requestUsername' : req.body.requestUsername } } , 
						...
						RequestCredential.deleteOne({'requestUsername' : req.body.requestUsername }).then((entryUpdate) => {	console.log(`Entry successfully deleted.`)	});	}
