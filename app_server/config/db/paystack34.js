const paystack = (axios) => {
		...
		const transactionLink = require('../url')['payment'];

		axios.defaults.headers.common['Authorization'] = mySecretKey;
	
	 const initializePayment = (formData , mycallback) => { let link = `${transactionLink}/initialize-transaction`;

			axios({	'method': 'get' , 'url' : link , 'data' : formData })
