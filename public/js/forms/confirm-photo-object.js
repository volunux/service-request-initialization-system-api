...
/** Form Handling Section below **/ 
	...
	$formProcess.onclick = function(e) {
				...
				var ulErrors1 = document.querySelector('ul.errors');
				...
				var $myData = {	'message' : document.getElementById('message').value , 'documents' : $$sFiles };

				if ($rmethod == 'POST') {

					function formValidity() {	

						if ($myData.message && $myData.message.length < 500) {
				...
				lii.textContent = 'Message body should be provided and cannot be empty.';
...
$xhrForm.open($rmethod , '/request' + $uaddress);
...
$xhrForm.onload = function(res) {
	...
	if (res.target.status == 200 || res.target.status == 201) {
			...
			setTimeout(() =>  window.location = `/request/${ulink}` , 3000);
